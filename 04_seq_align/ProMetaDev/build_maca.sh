...
#        （否则 __host__ __device__ 宏会退化成空）；
#     4. fast-math 开关是 -use-fast-math（连字符），不是 nvcc 的 -use_fast_math。
#
#   测试脚本 tests/run_tests.sh 固定用 ./build/seqalign，
#   可先 `mkdir -p build && cp build_maca/seqalign build/` 再跑。
...
mkdir -p "$BUILD_DIR"
echo "[build_maca] 编译 seqalign（5 个源文件，一次编译链接）"
"$MXCC" -x maca -offload-arch native --maca-path="$MACA" \
    ...
    -o "$BUILD_DIR/seqalign"

echo "构建完成: $ROOT/$BUILD_DIR/seqalign"
